...
export function PromptForm({ action, prompt }: Props) {
  ...
  return (
    <Formik
      ...
      onSubmit={async (values: PromptFormValues, { setSubmitting, resetForm }) => {
        await action(values);
        if (!prompt) resetForm();
        setSubmitting(false);
      }}
        ...
        <form onSubmit={handleSubmit} className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism">
          <VStack gap={6}>
            <FormField
              ...
              rows={5}
            />
            <FormField as="input" label="Chat Link" name={PromptFormFields.CHAT_URL} placeholder="Paste a chat link." />

            <FormSelect name={PromptFormFields.TAG} label="Select a category" placeholder="Choose an option">
          ...
          <div className="flex-end mx-3 mb-5 gap-4">
            ...
            <button className="px-5 py-1.5 text-sm bg-primary-orange rounded-full text-white" type="submit">
